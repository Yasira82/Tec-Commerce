...
export function AddProductForm({ onSuccess }: Props) {
  const [form, setForm] = useState<FormData>({
    ...
    images: '', country: 'Egypt', city: '',
    shipsTo: '', shippingCost: '0', estimatedDays: '3-7 days',
    ...
    <div style={{ background: '#0d0d14', border: '1px solid #d4af3720', borderRadius: 18, padding: 20 }}>
      ...
      {error && (
        <div style={{ color: '#e74c3c', fontSize: 12, marginBottom: 12, padding: '8px 12px',
          background: '#e74c3c10', borderRadius: 8 }}>
          {error}
        </div>
      )}
      ...
      {step === 1 && (
        <div>
          ...
          <input value={form.images} onChange={e => update('images', e.target.value)}
            placeholder="https://image1.jpg, https://image2.jpg" style={inputStyle} />
          ...
          <button onClick={() => setStep(2)}
            style={{ width: '100%', padding: '14px', borderRadius: 14,
              ...
              border: 'none', color: '#0a0800', fontSize: 14, fontWeight: 800, cursor: 'pointer' }}>
            Next: Shipping →
          </button>
        </div>
      )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div>
              ...
              <input type="number" min="0" step="0.01" value={form.shippingCost}
                ...
                placeholder="0 = Free" style={inputStyle} />
            </div>
            <div>
      ...
      {step === 3 && (
        <div>
          ...
          <div style={{ background: '#ffffff05', borderRadius: 12, padding: '12px', marginBottom: 14 }}>
            <div style={{ fontSize: 11, color: '#6b6b7a', marginBottom: 6 }}>📋 Product Summary:</div>
            <div style={{ fontSize: 12, color: '#fff' }}>{form.title || 'No title'}</div>
            <div style={{ fontSize: 12, color: '#d4af37', marginTop: 4 }}>{form.price || '0'}π</div>
            <div style={{ fontSize: 11, color: '#4a4a5a', marginTop: 4 }}>
              {form.city}, {form.country} · Ships in {form.estimatedDays}
            </div>
          </div>
